...
def animate_one_at_a_time(i: int):
    ...
    ax.fill_between(
        x, y, 0, where=(y > 0), facecolor="red", alpha=0.3, interpolate=True
    )
    # Area below 0 -> light blue
    ax.fill_between(
        x, y, 0, where=(y < 0), facecolor="blue", alpha=0.3, interpolate=True
    )
    ...
    inset_ax.plot(
        0.4 * np.array(times[: frame_pointer[0] + 1]),
        total_x_mag[: frame_pointer[0] + 1],
    )
    inset_ax.plot(
        0.4 * np.array(times[frame_pointer[0] + 1]),
        total_x_mag[frame_pointer[0] + 1],
...
ani = animation.FuncAnimation(
    fig, animate_one_at_a_time, frames=number_of_slices, interval=50
)
